...
internal class BookServiceTest {
    ...
    fun `getList when book list is exist then return list`() {
        ...
        // Then
        assertThat(result).`as`("検索した書籍リストが一致する").isEqualTo(expected)
    }
    ...
    fun `getList when book list is not exist then return empty list`() {
        ...
        // Then
        assertThat(result).`as`("書籍リストが空").isEmpty()
    }
    ...
    fun `getDetail when book is exist then return bookWithRental`() {
        ...
        val expected = BookWithRental(book, null)
        whenever(bookRepository.findWithRental(any() as Long)).thenReturn(expected)

        // When
        val result = bookService.getDetail(book.id)

        // Then
        assertThat(result).`as`("IDから書籍を検索する").isEqualTo(expected)
    }

    @Test
    @DisplayName("IDに該当する書籍がリストに無ければ例外発生")
    fun `getDetail when book is not exist then throw Exception`() {
        ...
        // Then
        assertThat(exception.message).`as`("例外メッセージ").isEqualTo("存在しない書籍ID: $bookId")
    }
}